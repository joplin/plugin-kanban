...
export function useRemoteBoard(): [BoardState | undefined, boolean, (action: Action) => void] {
...
export function useTempBoard(board?: BoardState): [BoardState | undefined, (noteId: string, oldCol: string, newCol: string) => void] {
  const tempState = useRef<{ movedNote?: NoteData, oldCol?: string, newCol?: string }>({})

  const moveNote = (noteId: string, oldCol: string, newCol: string) => {
    if (!board) return

    const note = (board.columns.find(({ name }) => name === oldCol)?.notes as NoteData[]).find(({ id }) => id === noteId) as NoteData
    tempState.current = {
      movedNote: note,
      oldCol, newCol
    }
  ...
  if (board && tempState.current.movedNote) {
    const tempBoardCols = board.columns.map(({ name, notes }) => {
      let newNotes = notes
      if (tempState.current.newCol === name) newNotes = [...notes, tempState.current.movedNote as NoteData]
